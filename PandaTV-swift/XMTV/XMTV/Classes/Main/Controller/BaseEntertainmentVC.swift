...
extension BaseEntertainmentVC : UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
      ...
      let anchor = baseVM.anchorGroups[indexPath.section].anchors[indexPath.item]
       ...
       print(anchor.reduce_ratio)
        
//        anchor.isVertical == 0 ? pushNormalRoomVc(anchor) : presentShowRoomVc(anchor)
        
//        var modell:LivelistModel? = nil;
//        modell=LivelistModel.init( dict: )
//      modell = baseVM.anchorGroups[indexPath.section].anchors[indexPath.item];
